...
-export([get_record_and_type_from_core/1,get_spec_info_from_core/3]).
...
get_record_and_type_info(AbstractCode) ->
  ...
  %% /RV
  %% Attrs = [ Attr || {attribute,_,_,_} = Attr <- AbstractCode ],
  %% io:format("as: ~p\n", [Attrs]),
...
get_record_and_type_from_core(Core) ->
    Mod = cerl:concrete(cerl:module_name(Core)),
    Attrs = cerl:module_attrs(Core),
    %% io:format("2a:~p\n", [{attrs,Attrs}]),	%rv
    get_record_and_type_from_core(Attrs, Mod, [], dict:new()).

get_record_and_type_from_core([{N,V}|Attrs], Mod, Recs, RecInfo0) ->
    case {cerl:concrete(N),cerl:concrete(V)} of
	{type,[[{record,Rec},Fs0,_]]} ->
	    {ok,Fs1} = get_record_fields(Fs0, RecInfo0),
	    %% io:format("2a:~p\n", [{record,Rec,fs,Fs1}]), %rv
	    Arity = length(Fs1),
	    RecInfo1 = dict:store({record,Rec}, [{Arity,Fs1}], RecInfo0),
	    get_record_and_type_from_core(Attrs, Mod, [{record,Rec}|Recs],
					  RecInfo1);
	_ ->
	    get_record_and_type_from_core(Attrs, Mod, Recs, RecInfo0)
    end;
get_record_and_type_from_core([], _Mod, Recs, RecInfo) ->
    case type_record_fields(lists:reverse(Recs), RecInfo) of
	{ok, _Type} = Ok ->
	    ?debug(_NewRecDict),
	    Ok;
	{error, Name, Error} ->
...
-spec get_spec_info_from_core(atom(), cerl:c_module(), dict:dict()) ->
				 {ok,spec_dict(),callback_dict()}.

get_spec_info_from_core(Core, Mod, Type) ->
    get_spec_info_from_core(Core, dict:new(), dict:new(),
			    Mod, Type, "nofile").

get_spec_info_from_core(_Core, Spec, Callback, _Mod, _Type, _File) ->
    {ok,Spec,Callback}.
