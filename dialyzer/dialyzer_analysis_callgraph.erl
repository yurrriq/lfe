    ...
    try
      ...
      RemMods =
        [case Analysis#analysis.start_from of
           ...
           src_code -> list_to_atom(filename:basename(F, ".lfe"))
         end || F <- Files],
...
start_compilation(File,
  ...
  case StartFrom of
    src_code ->
      compile_lfe(File, IncludeD, Defines, Callgraph, Codeserver, UseContracts);
    byte_code ->
...
compile_common(File, AbstrCode, CompOpts, Callgraph, CServer, UseContracts) ->
  case dialyzer_utils:get_core_from_abstract_code(AbstrCode, CompOpts) of
    ...
    {ok, Core} ->
      ...
      %% /RV
      %% io:format("1:~p\n", [{mod,Mod,core,Core,nowarn,NoWarn,callgraph,Callgraph}]),
		...
		{ok, SpecInfo, CallbackInfo} ->
		  CServer2 =
		    dialyzer_codeserver:store_temp_contracts(Mod, SpecInfo,
							     ...
							     CServer1),
		  %% /RV
		  %% io:format("3a:~p\n", [{specinfo,SpecInfo,callbackinfo,CallbackInfo,cserver2,CServer2}]),
...
compile_lfe(File, Includes, _Defines, Callgraph, CServer, _UseContracts) ->
    SrcCompOpts = dialyzer_utils:src_compiler_opts(),
    CompOpts = [return] ++ Includes ++ SrcCompOpts,
...
compile_lfe(Core, Callgraph, CServer) ->
    Mod = cerl:concrete(cerl:module_name(Core)),
    NoWarn = core_get_nowarn(Core, Mod),
    %% io:format("1:~p\n", [{mod,Mod,core,Core,nowarn,NoWarn,callgraph,Callgraph}]), %rv
    case dialyzer_utils:get_record_and_type_from_core(Core) of
	{ok,RecInfo} ->
	    %% io:format("2b:~p\n", [{recinfo,RecInfo}]), %rv
	    CServer1 = dialyzer_codeserver:store_temp_records(Mod, RecInfo,
							      CServer),
	    {ok,SpecInfo,CallbackInfo} =
		dialyzer_utils:get_spec_info_from_core(Core, Mod, RecInfo),
	    CServer2 = dialyzer_codeserver:store_temp_contracts(Mod, SpecInfo,
								CallbackInfo,
								CServer1),
	    %% io:format("3a:~p\n", [{specinfo,SpecInfo,callbackinfo,CallbackInfo,cserver2,CServer2}]), %rv
	    store_core(Mod, Core, NoWarn, Callgraph, CServer2)
    end.

core_get_nowarn(Core, Mod) ->
    Attrs = cerl:module_attrs(Core),
    Fun = fun ({N,V}) ->
		  case cerl:concrete(N) of
		      compile ->
			  cerl:concrete(V);
		      _ -> []
		  end
	  end,
    Copts = lists:flatmap(Fun, Attrs),		%Compiler options
    Warn = erl_lint:bool_option(warn_unused_function, nowarn_unused_function,
				true, Copts),
    %% io:format("cgn: ~p\n", [{Attrs,Copts,Warn}]), %rv
    case Warn of
	false ->
	    [ {Mod,F,A} || {N,_} <- cerl:module_defs(Core),
			   begin {F,A} = cerl:var_name(N), true end ];
	true ->
	    %% This format is LFE specific.
	    [ {Mod,F,A} || [nowarn_unused_function|FAs] <- Copts,
			   [F,A] <- FAs ]
    end.

%% This is copied from dialyzer_utils.
